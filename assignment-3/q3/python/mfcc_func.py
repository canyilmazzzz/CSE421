...
def create_mfcc_features(
    recordings_list, FFTSize, sample_rate, numOfMelFilters, numOfDctOutputs, window
):
    freq_min = 20
    ...
    for sample_idx, wav_path in enumerate(recordings_list):
        ...
        if len(sample < 2 * FFTSize):
            sample = np.pad(
                sample, (0, 2 * FFTSize - len(sample)), "constant", constant_values=0
            )
